 ...
 * File     : cilOperatorlogicalAnd.cpp
 * Date     : 
 * Version  : 
 *
 ****************************************************************************/
 ...
 * ECMA262 11.11
 ***************************************************************************/	
CCgNodeData* CCilCodeGen::logicalAndOperator( CCgNodeData* lhs, CCgNodeData* rhs )
{
	assert( lhs != NULL );
	assert( rhs != NULL );
	...
	Node = makeFormulaNode( CG_LOGICALAND, lhs, rhs, true, true );
 ...
 * emit logicalAnd Operation
  ***************************************************************************/	
CCgNodeData* CCilCodeGen::emitLogicalAndOperator( CCgNodeData* nodetree )
{
	...
	//Left hand side
	nodetree->lhsNode = parseExpressionNodeTree( ptr, false );
	...
	if( !m_iLabelEndLoop )
	{
			...
			|| iByteCode == CEE_CEQ )
		{
			//Regular pass
			emit( CEE_DUP );
			emit( CEE_BRFALSE, iLabelFalse );	//Pop 1
